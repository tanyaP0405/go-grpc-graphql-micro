...
import (
	...
	"github.com/tanyaP0405/go-grpc-graphql-micro/order/pb"
...
func (s *grpcServer) PostOrder(ctx context.Context, r *pb.PostOrderRequest) (*pb.PostOrderResponse, error) {
	...
	data, err := order.CreatedAt.MarshalBinary()
	if err != nil {
		return nil, err
	}
	orderProto.CreatedAt = data
...
func (s *grpcServer) GetOrdersForAccount(ctx context.Context, r *pb.GetOrdersForAccountRequest) (*pb.GetOrdersForAccountResponse, error) {
	...
	if err != nil {
		log.Println("Error fetching orders for account: ", err)
		return nil, errors.New("error fetching orders for account")
	}
	...
	for _, o := range accountOrders {
		...
		data, err := o.CreatedAt.MarshalBinary()
		if err != nil {
			return nil, err
		}
		op.CreatedAt = data
